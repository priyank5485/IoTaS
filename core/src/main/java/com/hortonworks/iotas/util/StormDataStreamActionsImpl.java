...
public class StormDataStreamActionsImpl implements DataStreamActions {
    ...
    public void init (Map<String, String> conf) {
        if (conf != null) {
            if (conf.containsKey(DataStreamLayoutConstants.STORM_ARTIFACTS_LOCATION_KEY)) {
                stormArtifactsLocation = conf.get(DataStreamLayoutConstants.STORM_ARTIFACTS_LOCATION_KEY);
            }
            stormJarLocation = conf.get(DataStreamLayoutConstants.STORM_JAR_LOCATION_KEY);
            catalogRootUrl = conf.get(DataStreamLayoutConstants.YAML_KEY_CATALOG_ROOT_URL);
        }
    ...
    synchronized private String createYamlFile (DataStream dataStream) throws
        ...
        try {
            ...
            yamlData.put(DataStreamLayoutConstants.YAML_KEY_NAME, this.getTopologyName(dataStream));
            addTopologyConfig();
            addDataSources();
            addDataSinks();
            addProcessors();
    ...
    private void addTopologyConfig () {
        ...
        config.put(DataStreamLayoutConstants.YAML_KEY_CATALOG_ROOT_URL, catalogRootUrl);
        config.put(DataStreamLayoutConstants.YAML_KEY_LOCAL_PARSER_JAR_PATH, "/tmp"); //TODO
        yamlData.put(DataStreamLayoutConstants.YAML_KEY_CONFIG, config);
    }
    ...
    private void addDataSources () {
       List<Map> dataSources = (List<Map>) jsonMap.get(
        ...
        for (Map dataSource: dataSources) {
            switch (DataStreamLayoutConstants.DataSourceType.valueOf
                    ((String) dataSource.get(DataStreamLayoutConstants
                            .JSON_KEY_TYPE))) {
                case KAFKA:
    ...
    private void addDataSinks () {
       List<Map> dataSinks = (List<Map>) jsonMap.get(
               DataStreamLayoutConstants.JSON_KEY_DATA_SINKS);
        for (Map dataSink: dataSinks) {
            switch (DataStreamLayoutConstants.DataSinkType.valueOf
                    ((String) dataSink.get(DataStreamLayoutConstants
                            .JSON_KEY_TYPE))) {
                case HBASE:
        ...
        grouping.put(DataStreamLayoutConstants.YAML_KEY_TYPE, "SHUFFLE");
        grouping.put(DataStreamLayoutConstants.YAML_KEY_STREAM_ID, DataStreamLayoutConstants.YAML_PARSED_TUPLES_STREAM);
    ...
    private void addProcessors () {
       List<Map> processors = (List<Map>) jsonMap.get(
               DataStreamLayoutConstants.JSON_KEY_PROCESSORS);
        for (Map processor: processors) {
            List<Map> processorComponents = (List) processor.get
                    (DataStreamLayoutConstants.JSON_KEY_CONFIG);
            ...
            if (processorComponents.size() == 2) {
                ...
                DataStreamLayoutConstants.ProcessorType processorType1 =
                        DataStreamLayoutConstants.ProcessorType.valueOf(
                                (String) processorComponent1.get
                                        (DataStreamLayoutConstants.JSON_KEY_TYPE));
                DataStreamLayoutConstants.ProcessorType processorType2 =
                        DataStreamLayoutConstants.ProcessorType.valueOf(
                                (String) processorComponent2.get
                                        (DataStreamLayoutConstants.JSON_KEY_TYPE));
                if (processorType1 == DataStreamLayoutConstants.ProcessorType
                        .RULE && processorType2 == DataStreamLayoutConstants
                        .ProcessorType.RULE) {
                    Integer ruleId1 = (Integer) processorComponent1.get
                            (DataStreamLayoutConstants.JSON_KEY_ID);
                    ...
                    if (ruleId1.equals(1) && ruleId2.equals(2)) {
                        this.goodBadTuplesProcessor = (String) processor.get
                                (DataStreamLayoutConstants.JSON_KEY_UINAME);
            ...
            for (Map processorComponent: processorComponents) {
                switch (DataStreamLayoutConstants.ProcessorType.valueOf
    ...
    private void addLinks () {
       List<Map> links = (List<Map>) jsonMap.get(
               DataStreamLayoutConstants.JSON_KEY_LINKS);
        for (Map link: links) {
            String linkFrom = (String) link.get(DataStreamLayoutConstants
                    .JSON_KEY_FROM);
            String linkTo = (String) link.get(DataStreamLayoutConstants
                    .JSON_KEY_TO);
            ...
            if (linkFrom.equals(badTuplesRule)) {
                streamId = DataStreamLayoutConstants.YAML_FAILED_TO_PARSE_TUPLES_STREAM;
            } else if (linkFrom.equals(goodTuplesRule)) {
                streamId = DataStreamLayoutConstants.YAML_PARSED_TUPLES_STREAM;
            }
            Map grouping = new LinkedHashMap();
            grouping.put(DataStreamLayoutConstants.YAML_KEY_TYPE, "SHUFFLE");
            if (!StringUtils.isEmpty(streamId)) {
                grouping.put(DataStreamLayoutConstants.YAML_KEY_STREAM_ID, streamId);
            }
    ...
    private void addKafkaDataSource (Map dataSource) {
        ...
        String dataSourceUiName  = (String) dataSource.get
                (DataStreamLayoutConstants.JSON_KEY_UINAME);
        Map config = (Map) dataSource.get(DataStreamLayoutConstants
                .JSON_KEY_CONFIG);
        ...
        zkHostsConstructorArgs.add(config.get(DataStreamLayoutConstants
                .JSON_KEY_ZK_URL));
        ...
        ref.put(DataStreamLayoutConstants.YAML_KEY_REF, zkHostsComponentId);
        spoutConfigConstructorArgs.add(ref);
        spoutConfigConstructorArgs.add(config.get(DataStreamLayoutConstants
                .JSON_KEY_TOPIC));
        ...
        ref.put(DataStreamLayoutConstants.YAML_KEY_REF, spoutConfigId);
        ...
        configParsedTuplesStreamMethod.put(DataStreamLayoutConstants.YAML_KEY_NAME,
                "withParsedTuplesStreamId");
        List configParsedTuplesStreamMethodArgs = new ArrayList();
        configParsedTuplesStreamMethodArgs.add(DataStreamLayoutConstants.YAML_PARSED_TUPLES_STREAM);
        configParsedTuplesStreamMethod.put(DataStreamLayoutConstants.YAML_KEY_ARGS,
        ...
        configUnparsedTuplesStreamMethod.put(DataStreamLayoutConstants.YAML_KEY_NAME,
                "withUnparsedTuplesStreamId");
        List configUnparsedTuplesStreamMethodArgs = new ArrayList();
        configUnparsedTuplesStreamMethodArgs.add(DataStreamLayoutConstants.YAML_FAILED_TO_PARSE_TUPLES_STREAM);
        configUnparsedTuplesStreamMethod.put(DataStreamLayoutConstants.YAML_KEY_ARGS,
    ...
    private void addHbaseDataSink (Map dataSink) {
        ...
        String dataSinkUiName  = (String) dataSink.get
                (DataStreamLayoutConstants.JSON_KEY_UINAME);
        Map config = (Map) dataSink.get(DataStreamLayoutConstants
                .JSON_KEY_CONFIG);
        ...
        hbaseMapperConstructorArgs.add(config.get(DataStreamLayoutConstants
        ...
        hbaseBoltConstructorArgs.add(config.get(DataStreamLayoutConstants
                .JSON_KEY_TABLE));
        Map ref = new LinkedHashMap();
        ref.put(DataStreamLayoutConstants.YAML_KEY_REF, hbaseMapperComponentId);
        hbaseBoltConstructorArgs.add(ref);
        List hbaseBoltConfigMethods = new ArrayList();
        Map configKeyMethod = new LinkedHashMap();
        configKeyMethod.put(DataStreamLayoutConstants.YAML_KEY_NAME, "withConfigKey");
        List configKeyMethodArgs = new ArrayList();
        configKeyMethodArgs.add(DataStreamLayoutConstants.YAML_KEY_HBASE_CONF);
        configKeyMethod.put(DataStreamLayoutConstants.YAML_KEY_ARGS, configKeyMethodArgs);
        ...
        Map topologyConfig = (LinkedHashMap) this.yamlData.get(DataStreamLayoutConstants.YAML_KEY_CONFIG);
        if (topologyConfig == null) {
            topologyConfig = new LinkedHashMap();
            this.yamlData.put(DataStreamLayoutConstants.YAML_KEY_CONFIG, topologyConfig);
        }
        Map hbaseConfMap = new LinkedHashMap();
        hbaseConfMap.put(DataStreamLayoutConstants.YAML_KEY_HBASE_ROOT_DIR, (String) config.get
                (DataStreamLayoutConstants.JSON_KEY_ROOT_DIR));
        topologyConfig.put(DataStreamLayoutConstants.YAML_KEY_HBASE_CONF, hbaseConfMap);
    }
    ...
    private void addHdfsDataSink (Map dataSink) {
        ...
        pathConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_NAME, "withPath");
        List pathMethodArgs = new ArrayList();
        pathMethodArgs.add(config.get(DataStreamLayoutConstants
                .JSON_KEY_PATH));
        pathConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_ARGS, pathMethodArgs);
        fileNameFormatConfigMethods.add(pathConfigMethod);

        Map nameConfigMethod = new LinkedHashMap();
        nameConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_NAME,
                "withPrefix");

        List nameMethodArgs = new ArrayList();
        nameMethodArgs.add(config.get(DataStreamLayoutConstants
                .JSON_KEY_NAME));
        nameConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_ARGS, nameMethodArgs);
        ...
        fsUrlConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_NAME, "withFsUrl");
        List fsUrlMethodArgs = new ArrayList();
        fsUrlMethodArgs.add(config.get(DataStreamLayoutConstants.JSON_KEY_FS_URL));
        fsUrlConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_ARGS, fsUrlMethodArgs);
        hdfsBoltConfigMethods.add(fsUrlConfigMethod);

        Map fileNameFormatConfigMethod = new LinkedHashMap();
        fileNameFormatConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_NAME,
                "withFileNameFormat");
        List fileNameFormatMethodArgs = new ArrayList();
        Map fileNameFormatRef = new LinkedHashMap();
        fileNameFormatRef.put(DataStreamLayoutConstants.YAML_KEY_REF,
                fileNameFormatId);
        fileNameFormatMethodArgs.add(fileNameFormatRef);
        fileNameFormatConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_ARGS,
                fileNameFormatMethodArgs);
        hdfsBoltConfigMethods.add(fileNameFormatConfigMethod);

        Map recordFormatConfigMethod = new LinkedHashMap();
        recordFormatConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_NAME,
                "withRecordFormat");
        List recordFormatMethodArgs = new ArrayList();
        Map recordFormatRef = new LinkedHashMap();
        recordFormatRef.put(DataStreamLayoutConstants.YAML_KEY_REF, recordFormatId);
        recordFormatMethodArgs.add(recordFormatRef);
        recordFormatConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_ARGS,
                recordFormatMethodArgs);
        hdfsBoltConfigMethods.add(recordFormatConfigMethod);

        Map rotationPolicyConfigMethod = new LinkedHashMap();
        rotationPolicyConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_NAME,
                "withRotationPolicy");
        List rotationPolicyMethodArgs = new ArrayList();
        Map rotationPolicyRef = new LinkedHashMap();
        rotationPolicyRef.put(DataStreamLayoutConstants.YAML_KEY_REF, rotationPolicyId);
        rotationPolicyMethodArgs.add(rotationPolicyRef);
        rotationPolicyConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_ARGS,
                rotationPolicyMethodArgs);
        hdfsBoltConfigMethods.add(rotationPolicyConfigMethod);

        Map syncPolicyConfigMethod = new LinkedHashMap();
        syncPolicyConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_NAME,
                "withSyncPolicy");
        List syncPolicyMethodArgs = new ArrayList();
        Map syncPolicyRef = new LinkedHashMap();
        syncPolicyRef.put(DataStreamLayoutConstants.YAML_KEY_REF, syncPolicyId);
        syncPolicyMethodArgs.add(syncPolicyRef);
        syncPolicyConfigMethod.put(DataStreamLayoutConstants.YAML_KEY_ARGS,
                syncPolicyMethodArgs);
    ...
    private Map createComponent (String id, String className, List
        ...
        component.put(DataStreamLayoutConstants.YAML_KEY_ID, id);
        component.put(DataStreamLayoutConstants.YAML_KEY_CLASS_NAME, className);
        if (constructorArgs != null && constructorArgs.size() > 0) {
            component.put(DataStreamLayoutConstants.YAML_KEY_CONSTRUCTOR_ARGS, constructorArgs);
        }
        if (configMethods != null && configMethods.size() > 0) {
            component.put(DataStreamLayoutConstants.YAML_KEY_CONFIG_METHODS, configMethods);
        }
        return component;
    }
    ...
    private Map createStream (String from, String to, Map grouping) {
        ...
        stream.put(DataStreamLayoutConstants.YAML_KEY_NAME, from + "->" + to);
        stream.put(DataStreamLayoutConstants.YAML_KEY_FROM, from);
        stream.put(DataStreamLayoutConstants.YAML_KEY_TO, to);
        stream.put(DataStreamLayoutConstants.YAML_KEY_GROUPING, grouping);
        return stream;
    }
    private void addToComponents (Map component) {
        this.addComponentToCollection(component, DataStreamLayoutConstants.YAML_KEY_COMPONENTS);
    }

    private void addToSpouts (Map spout) {
        this.addComponentToCollection(spout, DataStreamLayoutConstants.YAML_KEY_SPOUTS);
    }

    private void addToBolts (Map bolt) {
        this.addComponentToCollection(bolt, DataStreamLayoutConstants.YAML_KEY_BOLTS);
    }

    private void addToStreams (Map stream) {
        this.addComponentToCollection(stream, DataStreamLayoutConstants.YAML_KEY_STREAMS);
    }
